...
class TestIssueDocument:
    def test_issue_boleta_success(
        self, db, completed_sale, boleta_series, sample_items
    ):
    ...
    def test_issue_factura_success(
        self, db, completed_sale, factura_series, sample_items
    ):
        doc = BillingService.issue_document(
            sale_id=completed_sale.id,
            series_code="F001",
    ...
    def test_correlativo_increments_sequentially(
        self, db, completed_sale, boleta_series, sample_items, cashier
    ):
        ...
        doc2 = BillingService.issue_document(
            ...
            customer_address="",
            items=[
                {"description": "Agua", "quantity": 1, "unit_price": Decimal("2.50")}
            ],
        )
    ...
    def test_duplicate_billing_for_sale_raises_409(
        self, db, completed_sale, boleta_series, sample_items
    ):
        BillingService.issue_document(
            sale_id=completed_sale.id,
    ...
    def test_ose_failure_marks_as_sent(
        self, db, completed_sale, boleta_series, sample_items
    ):
    ...
    def test_ose_rejection_marks_as_rejected(
        self, db, completed_sale, boleta_series, sample_items
    ):
        with patch("apps.billing.services.get_ose_client") as mock_get:
            mock_client = MagicMock()
...
class TestVoidDocument:
    def test_void_accepted_document(
        self, db, completed_sale, boleta_series, sample_items
    ):
        doc = BillingService.issue_document(
            sale_id=completed_sale.id,
        ...
        voided = BillingService.void_document(
            document_id=doc.id, reason="Error en datos"
        )
        assert voided.status == BillingDocument.Status.VOIDED
        assert voided.voided_at is not None

    def test_void_already_voided_raises_409(
        self, db, completed_sale, boleta_series, sample_items
    ):
        doc = BillingService.issue_document(
            sale_id=completed_sale.id,
    ...
    def test_void_rejected_document_raises_400(
        self, db, completed_sale, boleta_series, sample_items
    ):
        with patch("apps.billing.services.get_ose_client") as mock_get:
            mock_client = MagicMock()
...
class TestCorrelativoConcurrency:
    ...
    def test_concurrent_issue_produces_unique_numbers(self, boleta_series, cashier):
        ...
        sale_a = SaleService.create_sale(
            cashier=cashier, items=[{"product_id": product.id, "quantity": 1}]
        )
