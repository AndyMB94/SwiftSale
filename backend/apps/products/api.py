...
def update_category(
    request: HttpRequest, category_id: uuid.UUID, payload: CategoryUpdateInput
):
    require_admin_or_supervisor(request)
...
def update_product(
    request: HttpRequest, product_id: uuid.UUID, payload: ProductUpdateInput
):
    require_admin_or_supervisor(request)
...
def adjust_inventory(
    request: HttpRequest, product_id: uuid.UUID, payload: InventoryAdjustInput
):
    require_admin_or_supervisor(request)
...
@router.get(
    ...
    response=list[InventoryMovementOut],
    auth=cookie_auth,
)
